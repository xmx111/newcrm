        ...
        $.ajax({
          ...
          success: function (g) {
            if (g.type == 'error' || g.type == 'warning' || g.type == 'failure') {
              Global.notify('error', g.smessage)
            } else {
              if (f) {
                f.call(c, g)
              }
        ...
        $.ajax({
          ...
          success: function (h) {
            if (h.type == 'error' || h.type == 'warning' || h.type == 'failure') {
              Global.notify('error', h.smessage)
            } else {
              if (g) {
                g.call(c, h)
              }
      ...
      $.fn.ajaxPostURL = function (d, g, f, c) {
        ...
        $.post(encodeURI(d), c.data, function (h, l) {
          ...
          if (h.type == 'success' || h.type == 'warning') {
            Global.notify(h.type, h.smessage);
            if (g) {
              g.call(e, h)
            }
          } else {
            if (h.sdata) {
              ...
              for (var j in h.sdata) {
                k.push(h.sdata[j])
              }
              Global.notify('error', k.join('<br>'), h.smessage)
            } else {
              Global.notify('error', h.smessage)
            }
            if (c.failure) {
                ...
                success: function (f) {
                    ...
                    if (f.type == 'success') {
                        ...
                        if (f.type == 'failure' || f.type == 'error') {
                            Global.notify(f.type, f.smessage);
                            if (a) {
                                a.call(c, f)
                            }
                        } else {
...
//        if (d.length == 0) {
        ...
        if (!hasModal) {
          ...
          e.push('<div class="modal-dialog" style="width:800px;max-height:600px;">');
