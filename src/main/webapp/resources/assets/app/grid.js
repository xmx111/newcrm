  ...
  return {
    initGridDefault: function () {
      ...
      $.extend($.jgrid.del, {
        ...
        afterComplete: function (d) {
          ...
          if (b.type == 'success') {
            top.$.publishMessage(b.smessage);
            c[0] = true
          } else {
            top.$.publishError(b.smessage);
            c[0] = false
          }
          return c
        },
            ...
            if (c.del) {
              ...
              if (n.p.toppager) {
                ...
                $(n) .jqGrid('navButtonAdd', n.p.toppager, {
                  caption: c.deltext || '',
                  title: c.deltitle || '删除所选行项',
                  buttonicon: c.delicon + (n.p.inlineNav.items == false ? ' hide' : ''),
                  id: e + '_ildel',
                  onClickButton: function () {
    ...
    initGrid: function (W, E) {
            ...
            addRowParams: {
              ...
              aftersavefunc: function (av, aw) {
                ...
                if (au.type == 'success' || au.type == 'warning') {
                  Global.notify(au.type, au.smessage);
                  var c = au.sdata.id;
                  ...
                  if (au.type == 'failure' || au.type == 'error') {
                    Global.notify('error', au.smessage)
                  } else {
              ...
              errorfunc: function (au, av) {
                ...
                Global.notify('error', c.smessage)
              }
            }
          },
          editParams: {
            ...
            aftersavefunc: function (au, ax) {
              ...
              if (L.editurl != 'clientArray') {
                ...
                if (c.type == 'success' || c.type == 'warning') {
                  Global.notify(c.type, c.smessage)
                } else {
          ...
          c.editoptions = $.extend(c.editoptions, {
            dataInit: function (aB) {
              ...
              if (aA.is('select')) {
                aA.select2({
                  openOnEnter: false,
                  placeholder: !!c.editoptions.initSelectValue ? c.editoptions.initSelectValue : '请选择...',
      ...
      if (L.pager) {
        ...
        if (!!L.delurl && L.inlineNav.items == false) {
            var del = $('<li><a href="javascript:;"><i class="fa fa-trash"></i> 删除数据</a></li>') .appendTo(X);
            del.children('a') .bind('click', function (ax) {
              $('.ui-icon-trash', $(L.pager)) .click()
            });
            O.push(del);
        }
        if (L.fullediturl) {
        ...
        if (O.length > 0) {
          $.each(O, function () {
            ...
            if (Util.notSmallViewport()) {
              ...
              var av = $('<button type="button" class="btn btn-xs blue" style="margin-left:5px"><i class="' + aw + '"></i> ' + (aw == 'fa fa-plus-square' ? '新增' : aw == 'fa fa-trash' ? '删除' : c) + '</button>') .appendTo(af.parent());
