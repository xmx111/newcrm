package com.ufo.erp.company.web;
...
@Controller("erpCompanyCompanyController")
@RequestMapping("/erp/company/")
@Description(code = "erp.company", value = "小贷公司设置")
@Secured("erp.company")
public class CompanyController extends PersistableController<Company, Integer> {
    
    /***
     * jsp file path
     */
    private static final String VIEWPATH = "/erp/company/";

    @Autowired
    private ICompanyService companyService;

    @Override
    public IBaseSpringDataService<Company, Integer> getEntityService() {
        return companyService;
    }
    
    @RequestMapping("index.htm")
    @Description("列表查询")
    @Secured("erp.company.index")
    public String index(HttpServletRequest request) {
        return this.toView("company-index");
    }

    @RequestMapping("inputTabs.htm")
    @Description({ "新增数据", "编辑数据" })
    @Secured({ "erp.company.add", "erp.company.edit" })
    public String inputTabs(Integer id, Boolean clone, ModelMap map) {
        if (id != null && id != Integer.MIN_VALUE)
            map.put("id", id);
        if (clone == Boolean.TRUE)
            map.put("clone", clone);
        return this.toView("company-inputTabs");
    }
    ...
    public String edit(Integer id, Boolean clone, ModelMap map) {
        ...
        if (clone == Boolean.TRUE)
            map.put("clone", clone);
        return this.toView("company-inputBasic");
    }
    ...
    @Description("编辑数据")
    @Secured("erp.company.edit")
    @ResponseBody
    public Object update(Company dto) {
    ...
    @Secured({ "erp.company.add", "erp.company.edit" })
    @ResponseBody
    public Object save(Company dto) {
        return super.save(dto);
    }

    @RequestMapping("delete.json")
    @Description("删除数据")
    @Secured("erp.company.delete")
    @ResponseBody
