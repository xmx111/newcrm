 ...
 * Copyright (c) 2012
 */
package com.ufo.core.pagination;
...
import com.ufo.core.utils.ConvertUtils;
...
public class PropertyFilter {
    ...
    private Object parseMatchValueByClassType(Class propertyClass, String value) {
        ...
        if (Enum.class.isAssignableFrom(propertyClass)) {
            ...
            return DateUtils.parseToTimestamp(value, DateUtils.FT_DATE);
        } else {
    ...
    //app分页
    public static Pageable buildAppPageableFromHttpRequest(HttpServletRequest request) {
        String limit = StringUtils.isBlank(request.getParameter("limit")) ? "10" : request.getParameter("limit");
        if (Integer.valueOf(limit) < 0) {
            return null;
        }
        String page = StringUtils.isBlank(request.getParameter("page")) ? "1" : request.getParameter("page");
        Sort sort = buildSortFromHttpRequest(request);
        return new PageRequest(Integer.valueOf(page) - 1, Integer.valueOf(limit), sort);
    }
